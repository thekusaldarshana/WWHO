...
4.  **ID Unification (`MetaVocab`):**
    *   Tiktoken IDs are kept as-is (range `0` to `200,000`).
    *   SGPE IDs are offset by `tiktoken_size` (range `200,000` to `200,000 + N`).
...
| :--- | :--- | :--- |
| `0` - `199,999` | Latin script, Code, Digits, Emojis | `tiktoken` (o200k_base) |
| `200,000` | SGPE Token ID 0 | `vocab.json` |
| `200,001` | SGPE Token ID 1 | `vocab.json` |
| ... | ... | ... |
| `200,000 + N` | SGPE Token ID N | `vocab.json` |
